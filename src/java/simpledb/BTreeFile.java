...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		//System.out.println("-----------------------------------------");
		// some code goes here
		if (pid.pgcateg()==2) return (BTreeLeafPage) this.getPage(tid,dirtypages,pid,perm);

		BTreeInternalPage page = (BTreeInternalPage) this.getPage(tid,dirtypages,pid,perm);
		Iterator<BTreeEntry> ite = page.iterator();
		Iterator<BTreeEntry> Reite = page.reverseIterator();
		BTreeEntry currentEntry = ite.next();
		Field currentField = currentEntry.getKey();

		if (f==null) {
			BTreePageId leftChild = currentEntry.getLeftChild();
			pid = leftChild;
			return (BTreeLeafPage) findLeafPage(tid,dirtypages,pid,perm,f);
		}
		//System.out.println("f: "+f.toString()+"   currentField: "+currentField);

		while (true){
			if (f.compare(Op.LESS_THAN_OR_EQ,currentField)) return findLeafPage(tid,dirtypages,currentEntry.getLeftChild(),perm,f);
			else{
				if (ite.hasNext()){
					currentEntry = ite.next();
					currentField = currentEntry.getKey();
				}
				else break;;
			}
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		Iterator<Tuple> ite = page.reverseIterator();
		BTreeLeafPage newPage = (BTreeLeafPage) getEmptyPage(tid,dirtypages,BTreePageId.LEAF);
		int tupleNum = 0;
		int totalNum = page.getNumTuples();
		Tuple currentTuple = null;
		//System.out.println(page.getMaxTuples());
		while(tupleNum<totalNum/2 && ite.hasNext()){
			currentTuple = ite.next();
		...
		//create the new entry to be insert in the parent page (at this point, the left and right child have been set)
		BTreeEntry newEntry = new BTreeEntry(currentTuple.getField(this.keyField),page.getId(),newPage.getId());

		//find the parent page
		BTreePageId oldParent = page.getParentId();
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid,dirtypages,oldParent,newEntry.getKey());

		//insert new entry to parent page
		...
		newPage.setRightSiblingId(oldright);
		newPage.setLeftSiblingId(page.getId());
		if (oldright!=null){
			BTreeLeafPage rightPage = (BTreeLeafPage) getPage(tid,dirtypages,oldright,Permissions.READ_WRITE);
			rightPage.setLeftSiblingId(newPage.getId());
		}

		//if (page.getId().pageNumber()==)

		//find the page to return
		if (field.compare(Op.LESS_THAN,currentTuple.getField(keyField))) return page;
		else return newPage;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		Iterator<BTreeEntry>ite = page.reverseIterator();
		int entryNum = 0;
		//System.out.println(page.getMaxEntries());
		int totalNum = page.getNumEntries();
		while (entryNum<totalNum/2 ){
			BTreeEntry currentEntry = ite.next();
		...
		//create the new entry to be added to the parent page(can not be equal to any entry in left or right sons)
		BTreeEntry oldEntry = ite.next();
		page.deleteKeyAndRightChild(oldEntry);
		BTreeEntry newEntry = new BTreeEntry(oldEntry.getKey(),page.getId(),newPage.getId());
		...
		//find the page to be returned
		if (field.compare(Op.LESS_THAN,newEntry.getKey())) return page;
		else return newPage;
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		//caculate number of entry to be moved
		//System.out.println("stealFromLeafPage");

		int number = (page.getNumTuples()+sibling.getNumTuples())/2 - page.getNumTuples();

		//stealing
		Iterator<Tuple> ite = null;
		if (isRightSibling){
			ite = sibling.iterator();
		}
		else ite = sibling.reverseIterator();
		Tuple tuple = null;
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int number = (page.getNumEntries()+leftSibling.getNumEntries())/2-page.getNumEntries();

		//first put parent entry!!!
		Iterator<BTreeEntry>iteLeft = leftSibling.reverseIterator();
		BTreeEntry firstLeftEntry = iteLeft.next();
		Iterator<BTreeEntry>iteRight = page.iterator();
		BTreeEntry firstRightEntry = iteRight.next();
		BTreeEntry fromParentEntry = new BTreeEntry(parentEntry.getKey(),firstLeftEntry.getRightChild(),firstRightEntry.getLeftChild());
		page.insertEntry(fromParentEntry);
		number--;

		//stealing
		iteLeft = leftSibling.reverseIterator();
		for (int i = 0 ;i<number; i++){
			BTreeEntry currentEntry = iteLeft.next();
			leftSibling.deleteKeyAndRightChild(currentEntry);
			page.insertEntry(currentEntry);
		}

		BTreeEntry nowLeftestEntry = iteLeft.next();
		parentEntry.setKey(nowLeftestEntry.getKey());
		leftSibling.deleteKeyAndRightChild(nowLeftestEntry);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int number = (page.getNumEntries()+rightSibling.getNumEntries())/2-page.getNumEntries();

		//first put parent entry!!!
		Iterator<BTreeEntry>iteRight = rightSibling.iterator();
		BTreeEntry firstRightEntry = iteRight.next();
		Iterator<BTreeEntry>iteLeft = page.reverseIterator();
		BTreeEntry firstLeftEntry = iteLeft.next();
		BTreeEntry fromParentEntry = new BTreeEntry(parentEntry.getKey(),firstLeftEntry.getRightChild(),firstRightEntry.getLeftChild());
		page.insertEntry(fromParentEntry);
		number--;
		...
		//stealing
		iteRight = rightSibling.iterator();
		for (int i = 0 ;i<number; i++){
			BTreeEntry currentEntry = iteRight.next();
			rightSibling.deleteKeyAndLeftChild(currentEntry);
			page.insertEntry(currentEntry);
		}

		BTreeEntry nowRightestEntry = iteRight.next();
		parentEntry.setKey(nowRightestEntry.getKey());
		rightSibling.deleteKeyAndLeftChild(nowRightestEntry);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//System.out.println("mergeLeafPage");

		deleteParentEntry(tid,dirtypages,leftPage,parent,parentEntry);

		Iterator<Tuple>ite = rightPage.iterator();
		while(ite.hasNext()){
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//System.out.println("mergeInternalPage");

		deleteParentEntry(tid,dirtypages,leftPage,parent,parentEntry);

		Iterator<BTreeEntry>left = leftPage.reverseIterator();
		BTreeEntry leftEntry = left.next();
		Iterator<BTreeEntry>right = rightPage.iterator();
		BTreeEntry rightEntry = right.next();

		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(),leftEntry.getRightChild(),rightEntry.getLeftChild());
